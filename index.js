...
const
    ...
    TunnelManager = require('./includes/tunnelManager');
...
// Start the internet tunnel so the APK can phone home without a hardcoded IP
let tunnelManager = new TunnelManager(CONST.control_port);
global.tunnelManager = tunnelManager;
if (process.env.L3MON_TUNNEL !== 'false') {
    tunnelManager.start().then((result) => {
        if (!result.error) {
            CONST.clientUrl = result.url;
            logManager.log(CONST.logTypes.success, "Tunnel Ready: " + result.url);
        } else {
            logManager.log(CONST.logTypes.error, "Tunnel Failed: " + result.error);
        }
    });
}
